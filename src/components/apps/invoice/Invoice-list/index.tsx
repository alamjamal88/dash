...
function InvoiceList() {
    ...
    const [filterdData, setFilterdData] = useState(invoices);
    ...
    useEffect(() => {
        if (invoices.length > 0) {
            const result = invoices.filter((data: any) => {
                // Match the search term and status
                const matchesSearch =
                    data.billFrom.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    data.billTo.toLowerCase().includes(searchTerm.toLowerCase());
                const matchesStatus = activeTab === 'All' || data.status === activeTab;
                return matchesSearch && matchesStatus;
            });
            setFilterdData(result);
            console.log('Filtered Result:', result);
        }
    }, [invoices, activeTab, searchTerm]); // Ensure correct dependencies
