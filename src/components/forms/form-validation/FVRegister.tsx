...
const validationSchema = yup.object({
    ...
    changepassword: yup
        .string()
        .test('passwords-match', 'Both password need to be the same', function (value) {
            const { password } = this.parent; // Access sibling field `password`
            if (password && password.length > 0) {
                return value === password; // Ensure `changepassword` matches `password`
            }
            return true; // Skip validation if `password` is not set
        })
        .required('Confirm password is required') // Ensure `changepassword` is filled
});
