...
const signInWithMicrosoft = () =>
    signInWithPopup(auth, microsoftProvider).catch((error) => handleSignInError(error, auth));
const signInWithGithub = () => signInWithPopup(auth, githubProvider).catch((error) => handleSignInError(error, auth));
const signInWithTwitter = () => signInWithPopup(auth, twitterProvider).catch((error) => handleSignInError(error, auth));

const supportedPopupSignInMethods = [GoogleAuthProvider.PROVIDER_ID, GithubAuthProvider.PROVIDER_ID];

const handleSignInError = async (error: any, auth: any) => {
...
const getProviderForSignInMethod = (signInMethod: any) => {
    switch (signInMethod) {
